...
| Слой | `GetBaseLayer` — только **группировка** строк в палитре `(слой + текст)` | **не** фильтр; слой нигде не отсекаем |
...
**Не менять:** логику группировки `(слой + текст)` в Accumulator.
...
Общий принцип: **автоматически**, по содержимому текста и геометрии рядом — **без** `key==N`, **без привязки к слою** (слой объекта не важен), без координат чертежа.

**Слой нигде не участвует в фильтрах C1–C4:** ни `TM_текст`, ни `0`, ни любой другой — проверяем только текст и геометрию.
...
- **не** фильтровать по имени слоя (`TM_текст`, `0` и т.д.) — слой **никогда** не критерий;
- **не** считать выноской текст с чем-либо до/после цифры — **во всех режимах** (выделение / модель / viewport), даже на «правильном» слое;
...
- `key == N`, фиксированные X/Y чертежа, отсев по **имени** слоя;
