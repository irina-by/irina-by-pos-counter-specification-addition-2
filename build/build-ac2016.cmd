@echo off
setlocal
REM Сборка PosCounter.Net для AutoCAD 2016 SP1+ — 2024 (.NET Framework 4.5.2+, net452).
REM В build\AutoCAD.props нужен AutoCADSdkDirNet46 (путь к AutoCAD 2016).

set "ROOT=%~dp0.."
set "PROJ=%ROOT%\PosCounter.Net\PosCounter.Net.csproj"
set "OUT_DIR=%ROOT%\PosCounter.Net\bin\x64\Release\net452"
set "DEPLOY_DIR=%ROOT%\dll 2016"

dotnet restore "%PROJ%"
if errorlevel 1 exit /b 1

dotnet build "%PROJ%" -c Release -f net452
if errorlevel 1 exit /b 1

if not exist "%OUT_DIR%\PosCounter.Net.dll" (
  set "OUT_DIR=%ROOT%\PosCounter.Net\bin\Release\net452"
)
